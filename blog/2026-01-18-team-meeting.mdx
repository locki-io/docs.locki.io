...
- **Jean-Noël aka jnXmas (@jnxmas)**: Project lead, architecture, N8N deployment, Firecrawl testing, documentation, political outreach
...
- **Crawl Tracking Database**: SQL Lite/SQL Model chosen
  ...
  - Owner: Jean-Noël will build during Victor's absence
...
- **Access**:
  - Jean-Noël has access to `Ocapistaine Dev` dashboard
...
- **Failsafe approach**: NOMIC primary + Mistral backup = "two tracks maximum" (Jean-Noël's preference given 4-week timeline)
...
- **Review process**: Jean-Noël reviews and merges into `dev`
...
| **Orchestration**     | N8N (Docker, viety.loki.io)                     | Jean-Noël          | 🟡 ~2h from deployment |
| **Crawl Tracking**    | SQL Lite + SQL Model                            | Jean-Noël          | 🔴 Design phase        |
| **LLM Providers**     | Gemini, Ollama, Mistral, OpenAI, Perplexity     | Team               | ✅ Multi-provider      |
| **Opik Setup**        | Single dashboard, Dev → Prod, multiple API keys | Jean-Noël          | 🟡 Access pending      |
| **OCR Libraries**     | pdf2ocr, Tabula, pypdf (test all 3)             | Victor → Jean-Noël | 🔴 Blocked on crawling |
...
3. Jean-Noël reviews and merges into `dev`
4. Jean-Noël tests merged pipeline on additional sources during Victor's absence
...
3. pypdf
   ...
   **Owner**: Victor (primary), Jean-Noël (testing during Victor's absence)
...
- **Owner**: Jean-Noël (@jnxmas)
- **Description**: Create/configure `Ocapistaine Dev` Opik project, generate separate API keys for Jean-Noël/Guru/Victor, grant dashboard access to all team members, document key assignment and usage tracking approach
...
- **Owner**: Victor (@zcbtvag) + Jean-Noël (review)
- **Description**: Victor signals PR ready, Jean-Noël reviews and merges into `dev` branch, test merged pipeline functionality
...
- **Owner**: Victor (@zcbtvag) → Jean-Noël (if Victor unavailable)
...
- **Description**: Read architecture blog post Jean-Noël created (in `ocapistaine` repo), suggest additions specifically around NOMIC embedding implementation details, RAG service integration points, any conflicts/improvements from experience
...
| --------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------- | ------------------ | -------------------------------- |
| **Guru missing from stack discussion**                          | Medium   | Record meetings, request written feedback on tasks, schedule follow-up calls                                          | Jean-Noël          | 🟢 Mitigated (recordings shared) |
| **Victor unavailable Thu-Sun**                                  | Medium   | Victor completes Firecrawl by Wed, Jean-Noël tests during absence                                                     | Victor + Jean-Noël | 🟢 Planned                       |
...
| **N8N deployment blocking agent testing**                       | High     | Jean-Noël prioritizes ~2h completion ASAP                                                                             | Jean-Noël          | 🟡 In progress                   |
| **Opik access blocking Guru's development**                     | High     | Jean-Noël grants access within 24h (need Guru's username)                                                             | Jean-Noël          | 🟡 In progress                   |
| **Vector store decision blocking RAG**                          | Medium   | VectorStore + NOMIC confirmed; fallback = Mistral Studio                                                              | Guru + Jean-Noël   | 🟢 Resolved                      |
| **OCR quality unknown for scanned PDFs**                        | Medium   | Test 3 libraries (pdf2ocr, Tabula, pypdf), compare outputs                                                            | Victor → Jean-Noël | 🟡 Planned                       |
...
| **First agent not live before contributions deadline (~Feb 1)** | Critical | Fast-track Charter agent development this week, daily standups                                                        | Guru + Jean-Noël   | 🔴 Active risk                   |
| **Low citizen engagement with contributions**                   | Medium   | Consider UX simplification, focus on chatbot quality over contribution volume                                         | Jean-Noël          | 🟡 Monitoring                    |
...
| **French language hallucinations/bias in chatbot**              | Critical | Mistral backup, heavy Opik guardrails, manual testing in French                                                       | Guru + Jean-Noël   | 🟡 Design phase                  |
...
- Task ownership clear and distributed well (Guru = agents, Victor = crawling, Jean-Noël = infrastructure/orchestration)
...
- **Team availability gaps**: Victor's Thu-Sun absence, Guru's jet lag, Jean-Noël's teaching commitment
...
1. **N8N + Opik access**: Confirm both unblocked (Jean-Noël updates)
2. **Firecrawl PR status**: Review and merge if ready (Victor + Jean-Noël)
...
5. **Crawl tracking DB**: Review schema design (Jean-Noël)
...
1. **🔴 URGENT**: Complete N8N deployment (Jean-Noël, <48h) - blocks agent testing
2. **🔴 URGENT**: Set up Opik team access (Jean-Noël, <24h) - blocks Guru's development
...
- **Jean-Noël**: Full availability, prioritizing N8N/Opik/testing during Victor's absence
...
1. Jean-Noël completes N8N deployment (~2 hours)
2. Jean-Noël sets up Opik team access (needs Guru + Victor usernames via Discord)
...
6. Victor completes Firecrawl PR by Wednesday
7. Jean-Noël reviews and merges Firecrawl PR
8. Guru develops Charter validation agent, provides update in 1-2 days
9. Guru develops RAG agent prototype, provides update in 1-2 days
10. Jean-Noël tests Firecrawl on additional sources during Victor's absence
11. Jean-Noël begins crawl tracking DB with SQL Model
12. Guru + Victor review architecture blog post, suggest improvements
13. All team members familiarize with Opik capabilities
    **Next Week (Week 2 of Sprint)**:
14. OCR library testing begins (Victor returns, or Jean-Noël covers)
15. Document Service requirements defined collaboratively
16. RAG agent fully functional with Opik guardrails
17. Charter agent deployed to N8N workflows
18. Crawl tracking DB prototype operational
19. Architecture frozen, team shifts to integration phase
    **Week 3-4 (Final Sprint)**:
20. Full pipeline integration testing
21. French language validation and bias checking
22. Citizen-facing chatbot MVP deployment
23. Opik showcase polish (metrics, traces, dashboards, experiment tracking)
24. Hackathon submission preparation
25. Final outreach to 4th electoral list
26. Documentation finalization
